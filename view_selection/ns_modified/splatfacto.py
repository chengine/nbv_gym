...
class SplatfactoModelConfig(ModelConfig):
    ...
    learn_object_mask: bool = False
    """Option to learn object masks."""
...
class SplatfactoModel(Model):
    ...
    def prepare_rasterizer(
        ...
        # print(optimized_camera_to_world.shape)
        ...
        sam_masks = (
            sam_masks_crop.clone().requires_grad_(True)
            if self.config.learn_object_mask
            else None
        )
    ...
    def compute_diag_H_rgb_depth(
        self, camera: Cameras, compute_rgb_H=False, is_touch=False
    ):
        """
        ...
        if is_touch:
            ...
            sam_masks_prob = torch.where(
                sam_masks_prob > 0.5,
                torch.ones_like(sam_masks_prob),
                torch.zeros_like(sam_masks_prob),
            )
