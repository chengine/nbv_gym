        ...
        try:
            ...
            if not strict:
                ...
                if isinstance(self.model, NerfactoModel):
                    num_cams = len(self.datamanager.train_dataset.cameras)
                    # update the relevant parameters
                    model_field_embedding_appearance_weight_temp = self.model.field.embedding_appearance.embedding.weight
                    model_camera_optimizer_pose_adjustment_temp = self.model.camera_optimizer.pose_adjustment
                    
                    # resize the params
                    del self.model.field.embedding_appearance.embedding.weight
                    del self.model.camera_optimizer.pose_adjustment
                    # self.model.field.embedding_appearance.embedding.weight = torch.nn.Parameter(model_state["field.embedding_appearance.embedding.weight"].to(self.device))
                    # self.model.camera_optimizer.pose_adjustment = torch.nn.Parameter(model_state["camera_optimizer.pose_adjustment"].to(self.device))
                    
                self.model.load_state_dict(model_state, strict=False)
                
                if isinstance(self.model, NerfactoModel):
                    # update the relevant parameters
                    self.model.field.embedding_appearance.embedding.weight = model_field_embedding_appearance_weight_temp
                    self.model.camera_optimizer.pose_adjustment = model_camera_optimizer_pose_adjustment_temp
            else:
                raise
...
class VanillaPipeline(Pipeline):
    ...
    def compute_hessian(self, ray_bundle, rgb_weight):
                ...
                if output_path is not None:
                    for key in image_dict.keys():
                        ...
                        vutils.save_image(
                            ...
                            output_path / f"eval_{key}_{idx:04d}.png",
                        )
    ...
    def fisher_calc_from_views(
        ...
        for idx, cam in tqdm(enumerate(training_cams),total=len(training_cams), desc="Computing Hessian"):
            if sample_cam is None:
                sample_cam = cam

            cam = cam.to(self.device)
            # print(f"Processing training cam index: {idx}")
            cur_H: torch.tensor = self.compute_hessian(cam, rgb_weight)  # type: ignore
        ...
        for idx, view_pos in enumerate(
            tqdm(candidate_views, desc="Computing acq scores")
        ):
            # copy sample_cam and update the pose
            cam = copy.deepcopy(sample_cam)
            if camera_info is not None:
                cam.fx[:] = camera_info["fx"]
                cam.fy[:] = camera_info["fy"]
                cam.cx[:] = camera_info["cx"]
                cam.cy[:] = camera_info["cy"]
                cam.width[:] = camera_info["w"]
                cam.height[:] = camera_info["h"]
        ...
        # print("Selected view(s):", selected_idx)
