...
from planner_utils_bayes import *
...
def run_eval(
    ...
    if not view_cached_results:
        ...
        # image-to-pose reduction mode for uncertainty
        ...
        try:
            for i in tqdm(range(num_data_aug_steps), desc="Data Augmentation"):
                ...
                # Only extract the uncertainty from the NeRF once per (re)training phase.
                output_path = str(config_path.parent / "uncertainty_map.npy")
                extract_uncertainty(
                ...
                # load the Hessian
                hessian = torch.tensor(np.load(str(output_path))).to(method.device)
                ...
                for j in range(num_poses_to_add_per_augmentation_step):
                    ...
                    # best_pose, best_idx, acq_score = evaluate_next_view(
                    #     method, root_pose, candidate_poses, rgb_weight=1.0
                    
                    # # Create a directory for uncertainty renders
                    # unc_renders = f"unc_renders_{j}"
                    # unc_renders_dir = Path(output_path).parent / unc_renders
                    # unc_renders_dir.mkdir(parents=True, exist_ok=True)

                    # best_pose, best_idx = evaluate_bayes_uncertainty(
                    #     method, 
                    #     config_path=config_path,
                    #     data_dir=data_dir,
                    #     unc_path=output_path,
                    #     output_path=unc_renders_dir,
                    #     # current_pose=root_pose,
                    #     training_poses=candidate_poses,
                    #     camera_info=cam_intrinsics,
                    # )
                    
                    # camera object for the poses
                    candidate_cameras, *_ = method.generate_output_cameras(
                        candidate_poses,
                        **cam_intrinsics
                    )
