...
class ShadowSplatModel(SplatfactoModel):
    ...
    def get_outputs(
        ...
        if not self.training:
            ...
            fig_diag_img = -render[:, ..., 6:7].squeeze(
                0
            )  # NOTE: We negate this because we want to maximize this quantity, but view selection minimizes
            view_fig_diag_img = -render[:, ..., 7:8].squeeze(
                0
            )  # NOTE: We negate this because we want to maximize this quantity, but view selection minimizes
        else:
            coverage = None
