...
class ViewSelectionPipelineConfig(VanillaPipelineConfig):
    ...
    """Fixed number of steps between adding views. Ignored if add_every_n_steps_per_view is set."""
    add_every_n_steps_per_view: Optional[float] = None
    """Number of steps per view in active set between adding views. If set, calculates steps as current_dataset_size * this_value. Takes precedence over add_every_n_steps."""
    add_num_views: int = 1
    ...
    """Maximum number of views to use in the active set. If None, no limit is enforced."""
    ...
    kdtree_num_nearest_neighbors: int = 3
...
class ViewSelectionPipeline(VanillaPipeline):
    def __init__(
        ...
        config.datamanager.max_num_views = config.max_num_views
        ...
        if config.view_selector is not None and hasattr(self.datamanager, "view_selector"):
            ...
            if config.view_selector == "optics":
                view_selector = create_view_selector(
                    ...
                    use_kdtree_filter=config.use_kdtree_filter,
                    num_nearest_neighbors=config.kdtree_num_nearest_neighbors,
                )
            else:
                view_selector = create_view_selector(config.view_selector, use_kdtree_filter=config.use_kdtree_filter,
                    num_nearest_neighbors=config.kdtree_num_nearest_neighbors)
    ...
    def get_train_loss_dict(self, step: int):
        ...
        # Calculate steps between additions (either fixed or per-view)
        steps_between_additions = None
        if (
            hasattr(self.config, "add_every_n_steps_per_view")
            and self.config.add_every_n_steps_per_view is not None
        ):
            # Use per-view calculation: current_dataset_size * N
            current_dataset_size = len(self.datamanager.active_train_indices)
            steps_between_additions = int(
                current_dataset_size * self.config.add_every_n_steps_per_view
            )
        elif hasattr(self.config, "add_every_n_steps") and self.config.add_every_n_steps > 0:
            # Use fixed steps
            steps_between_additions = self.config.add_every_n_steps

        # Expand active set on schedule
        if (
            steps_between_additions is not None
            and steps_between_additions > 0
            and step > 0
            and step % steps_between_additions == 0
