...
from shadow_splat.shadow_splat_rendering import lumen_rasterization, chebyshev_weighting, render_equirect_from_sh, render_equirect_from_sg, focal_bce
...
class ShadowSplatModel(SplatfactoModel):
    ...
    def get_outputs(
        ...
        if irradiance is not None:
            ...
            # shadow_img = render[:, ..., -2:-1]
            # exposure = torch.mean(shadow_img) * torch.exp(self.light_params["intensity"])

            # relit_intensity = relit_intensity * exposure
