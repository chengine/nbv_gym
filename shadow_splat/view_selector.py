...
class OpticsViewSelector(ViewSelector):
    ...
    def select_views(
        ...
        if isinstance(model, ShadowSplatModel):
            if self.coverage_metric in ["coverage", "fig", "view_fig", "fig_diag", "view_fig_diag", "coverage_lit", "fig_color_field"]:
                ...
                elif self.coverage_metric in ["fig_color_field"]:
                    if len(active_indices) > 0:
                        training_cameras = [all_cameras[idx : idx + 1] for idx in active_indices]
                        model.update_visibility(training_cameras)
                    else:
                        # If there are no active views, we don't need to do anything
                        pass
