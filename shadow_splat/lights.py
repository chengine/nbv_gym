...
@dataclass
class LightOptimizerConfig(InstantiateConfig):
    """Configuration of optimization for camera poses."""

    _target: Type = field(default_factory=lambda: LightOptimizer)
...
class LightOptimizer(nn.Module):
    """Layer that modifies camera poses to be optimized as well as the field during training."""
    config: LightOptimizerConfig
    ...
    def apply_to_camera(self, camera: Cameras) -> torch.Tensor:
        ...
        adj = self(torch.tensor([0], dtype=torch.long)).to(camera.device)  # type: ignore

        print("Light optimizer adjustment", adj)
    ...
    def get_loss_dict(self, loss_dict: dict) -> None:
        """Add regularization"""
        if self.config.mode != "off":
            loss_dict["light_opt_regularizer"] = (
    ...
    def get_metrics_dict(self, metrics_dict: dict) -> None:
        ...
        if self.config.mode != "off":
            ...
            metrics_dict["light_opt_translation_max"] = trans.max()
            metrics_dict["light_opt_translation_mean"] = trans.mean()
            metrics_dict["light_opt_rotation_mean"] = np.rad2deg(rot.mean().cpu())
            metrics_dict["light_opt_rotation_max"] = np.rad2deg(rot.max().cpu())
    ...
    def get_param_groups(self, param_groups: dict) -> None:
        ...
        if self.config.mode != "off":
            ...
            param_groups["light_opt"] = camera_opt_params
        else:
            assert len(camera_opt_params) == 0